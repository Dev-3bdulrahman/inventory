<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('inventory::inventory.warehouses') }}</h2>
            <p class="text-gray-500 text-sm mt-1">{{ __('inventory::inventory.manage_warehouses') }}</p>
        </div>
        <button wire:click="openCreateModal"
            ...
            <span>{{ __('inventory::inventory.add_warehouse') }}</span>
        </button>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 mb-6 shadow-sm">
        <div class="flex flex-wrap items-end gap-4">
            ...
            <div class="w-full sm:w-auto sm:min-w-[160px]">
                <label class="block text-xs font-bold text-gray-400 mb-1.5 uppercase">{{ __('inventory::inventory.status') }}</label>
                <select wire:model.live="statusFilter"
                    ...
                    <option value="">{{ __('inventory::inventory.status') }}</option>
                    <option value="active">{{ __('inventory::inventory.active') }}</option>
                    <option value="inactive">{{ __('inventory::inventory.inactive') }}</option>
                </select>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-right border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('inventory::inventory.warehouse_code') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('inventory::inventory.warehouse_name') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('inventory::inventory.branch') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('inventory::inventory.is_main') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">{{ __('inventory::inventory.status') }}</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-center">{{ __('inventory::inventory.actions') }}</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">
                                @if($warehouse->is_main)
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400">
                                        {{ __('inventory::inventory.is_main') }}
                                    </span>
                                @else
                                    -
                                @endif
                            ...
                            <td class="px-6 py-4">
                                ...
                                <button wire:click="toggleStatus({{ $warehouse->id }})"
                                    class="px-2.5 py-1 rounded-full text-xs font-semibold transition-all duration-200 hover:opacity-80 active:scale-95 {{ $statusClasses[$warehouse->status] ?? $statusClasses['active'] }}">
                                    {{ __('inventory::inventory.' . $warehouse->status) }}
                                </button>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <button wire:click="openEditModal({{ $warehouse->id }})" title="{{ __('inventory::inventory.edit_warehouse') }}"
                                    ...
                                    <button 
                                        wire:click="$dispatch('swal:confirm', { 
                                            title: '{{ __('inventory::inventory.delete') }}',
                                            text: '{{ __('inventory::inventory.delete_confirm') }}',
                                            onConfirm: 'delete',
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <span>{{ __('inventory::inventory.no_warehouses') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-6 border-b border-gray-50 dark:border-gray-800 pb-4">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                                {{ $warehouseId ? __('inventory::inventory.edit_warehouse') : __('inventory::inventory.add_warehouse') }}
                            </h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('inventory::inventory.warehouse_code') }} *</label>
                                <input type="text" wire:model="code" class="w-full py-2 px-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white">
                                @error('code') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('inventory::inventory.warehouse_name') }} *</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('inventory::inventory.status') }} *</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('inventory::inventory.branch') }}</label>
                                <select wire:model="branch_id" class="w-full py-2 px-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white">
                                    <option value="">{{ __('inventory::inventory.select_branch') }}</option>
                            ...
                            <div class="flex items-center mt-6">
                                <input id="is_main" type="checkbox" wire:model="is_main" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
                                <label for="is_main" class="mr-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('inventory::inventory.is_main') }}</label>
                                @error('is_main') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('inventory::inventory.address') }}</label>
